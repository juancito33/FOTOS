...
class BaseApp(QWidget):
    ...
    def _build_campos(self, layout: QVBoxLayout):
        ...
        # Botón Guardar NPN - AHORA EN LA MISMA LÍNEA
        ...
        if tipo == "Convencional":
            if not any(self.clasificaciones.values()):
                QMessageBox.warning(self, "Imágenes faltantes", 
                                   "Debes seleccionar al menos una imagen antes de guardar la unidad.")
                return
        else:
            if not self.no_conv_fachada and not self.anexos:
                QMessageBox.warning(self, "Imágenes faltantes", 
                                   "Debes seleccionar al menos una imagen (fachada o anexo) antes de guardar la unidad.")
                return
...
class IndividualApp(MasivoApp):
    def __init__(self):
        ...
        # Cambiar color del botón ZIP a rojo
        self.btn_zip.setStyleSheet("background-color: green; color: white; font-weight: bold;")
    ...
    def create_zip(self):
        ...
        if tipo == "Convencional":
            if not any(self.clasificaciones.values()):
                QMessageBox.warning(self, "Imágenes faltantes", 
                                   "Debes seleccionar al menos una imagen antes de generar el ZIP.")
                return
        else:
            if not self.no_conv_fachada and not self.anexos:
                QMessageBox.warning(self, "Imágenes faltantes", 
                                   "Debes seleccionar al menos una imagen (fachada o anexo) antes de generar el ZIP.")
                return
        ...
        with ZipFile(zip_path, "w") as zf:
            for root, _, files in os.walk(carpeta):
                for f in files:
                    abs_f = os.path.join(root, f)
                    rel_f = os.path.relpath(abs_f, os.path.dirname(carpeta))
                    zf.write(abs_f, rel_f)
