import pandas as pd, unicodedata, os, warnings, time
...
df = pd.read_excel(XLSX_PATH, sheet_name=SHEET_IDX, dtype={"npn": str, "destino": str})
...
# ----------------------------------------------------------- #
print("→ Evaluando tipo y área...")
df["ok_area_tipo"] = ~(df["area_construida"].notna() & df["tipo"].isna())

# ----------------------------------------------------------- #
# 6 · alfa_carto_id único
# ----------------------------------------------------------- #
print("→ Evaluando alfa_carto_id por npn/resolucion...")
...
# 7 · Regla destino
# ----------------------------------------------------------- #
print("→ Evaluando destino económico...")
...
# 9 · Agregado por NPN
# ----------------------------------------------------------- #
print("→ Agregando resumen por NPN...")
...
with pd.ExcelWriter(VALID_XLS, engine="xlsxwriter") as w:
    validos.to_excel(w, index=False, sheet_name="validos_VISI_CARTAGO")

with pd.ExcelWriter(BAD_XLS, engine="xlsxwriter") as w:
    inconsistentes.to_excel(w, index=False, sheet_name="incON_VISIT_CARTAGO")

print("✔️ ¡Listo! Archivos exportados correctamente.")
print("⏱️ Duración total:", round(time.time() - start, 2), "segundos")