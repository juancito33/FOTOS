...
with pd.ExcelWriter(XLS_REP,engine="xlsxwriter") as w:
    resumen.to_excel(w,index=False,sheet_name="duplicados_resumen")
...
if not sin_doc.empty: sin_doc.to_excel(XLS_SIND,index=False,sheet_name="sin_documento")
if not sin_nom.empty: sin_nom.to_excel(XLS_VAC,index=False,sheet_name="sin_nombre")
